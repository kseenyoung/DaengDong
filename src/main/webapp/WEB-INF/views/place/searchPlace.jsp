...
<script>
    ...
    document.getElementById("day").addEventListener("click", function (event) {
        ...
        updateMapMarkers(selectedDay); // 지도에 핀 업데이트

    }
});

let currentMarkers = []; // 현재 표시 중인 마커를 저장
...
function updateMapMarkers(day) {
    ...
    // 선택된 일차의 장소 리스트
    const places = dayPlans[day];

    // 새로운 마커 추가
    places.forEach((place, idx) => {
        const position = new kakao.maps.LatLng(place.lat, place.lng);
        const marker = addMarker(position, idx, place.title, place);
